...
def test_prepare_single_pool():
    ...
    assert_close(R1, 1.0 / (T1 + EPSILON))
    assert_close(R2, 1.0 / (T2 + EPSILON))
...
def test_prepare_two_pool_bm():
    ...
    R1, R2, exchange_matrix, weight = prepare_two_pool_bm(T1a, T1b, T2a, T2b, weight_b, k)
...
def test_prepare_two_pool_mt():
    ...
    assert_close(R2, 1.0 / (T2a + EPSILON))
...
def test_prepare_three_pool():
    ...
    R1, R2, exchange_matrix, weight = prepare_three_pool(T1a, T1b, T2a, T2b, k1, k2, weight_b, weight_c)
